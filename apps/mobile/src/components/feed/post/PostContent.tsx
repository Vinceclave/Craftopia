...
export const PostContent: React.FC<PostContentProps> = memo(({ title, content, image_url, tags = [] }) => (
  <View className="mb-3">
    <Text className="font-medium text-craftopia-textPrimary mb-1 text-base">{title}</Text>
    ...
    {tags.length > 0 && (
      <View className="flex-row mb-3">
        {tags.slice(0, 3).map((tag, idx) => (
          <Text key={`${tag}-${idx}`} className="text-xs text-craftopia-secondary mr-3">#{tag}</Text>
        ))}
      </View>
    )}
  </View>
